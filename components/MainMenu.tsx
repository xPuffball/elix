...
import { BookOpen, Settings, Info, LogIn } from 'lucide-react';
        ...
        <div className="absolute inset-0 z-50 pointer-events-auto flex items-center justify-center">
            ...
            <div className="relative z-10 flex flex-col items-center gap-2 max-w-md w-full px-8">
                ...
                <div className="mb-4 flex flex-col items-center">
                    ...
                    {/* Title */}
                    <h1
                        className="font-display font-bold text-5xl text-white tracking-wide"
                        style={{
                            textShadow: '0 3px 0 #5D4037, 0 6px 12px rgba(0,0,0,0.4)',
                        }}
                    >
                        CozyClassroom
                    </h1>
                    ...
                    {buttons.map((btn) => {
                        ...
                        return (
                            <button
                                ...
                                className={`
                                    relative w-full py-3.5 px-6 rounded-2xl font-display font-bold text-lg
                                    transition-all duration-200 active:scale-95
                                    flex items-center justify-center gap-3
                                    ${btn.primary
                                        ? 'bg-cozy-green text-white shadow-lg shadow-green-900/30 border-2 border-white/30'
                                        : 'bg-cozy-brown/85 text-white/90 shadow-md border-2 border-white/15'
                                    }
                                    ${isHovered
                                        ? btn.primary
                                            ? 'scale-105 shadow-xl shadow-green-900/40 bg-green-500'
                                            : 'scale-105 shadow-lg bg-cozy-brown'
                                        : ''
                                    }
                                `}
                            >
                ...
                {/* Version */}
                <p className="mt-8 text-white/30 text-xs font-display tracking-widest">v0.1.0 alpha</p>
            </div>
        </div>
    );
};
